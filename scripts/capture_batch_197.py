...
ROUTES=['/n-f-t-wallet','/n-s-f-w-platform','/narrative-engine','/net-worth-tracker','/network-graph','/not-found','/notes-app','/notification-center','/notification-history','/notification-preferences','/notification-settings','/notifications','/notifications-center','/number-input-form','/nutrition-tracker','/o-auth-providers','/onboarding-tutorial','/order-book','/order-confirmation','/order-placement']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-197'
...
print(json.dumps({'batch':197,'routes':len(ROUTES),'modes':['desktop','mobile']}))
