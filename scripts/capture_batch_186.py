...
ROUTES=['/dating-profile-setup','/dating-subscription','/de-fi','/decentralized-identity','/delete-account','/delete-content','/department-management','/dependency-graph','/deployment-pipeline','/deprecation-policy','/derivative-trading','/derivatives-trading','/destination-guide','/developer-area','/developer-community','/developer-marketplace','/difficulty-tracking','/direct-messages','/direct-messaging','/disaster-recovery']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-186'
...
print(json.dumps({'batch':186,'routes':len(ROUTES),'modes':['desktop','mobile']}))
