...
ROUTES = [
    "/bridge-transactions", "/browser-extension", "/budget-planner", "/bug-reporting",
    "/bulk-operations", "/bulk-upload", "/c-c-p-a", "/c-d-n-management",
    "/cache-management", "/calculator", "/calendar", "/calendar-view",
    "/campaign-analytics", "/campaign-builder", "/campaign-creation", "/car-rental",
    "/card-grid-view", "/cash-flow-analysis", "/category-management", "/certificate-manager",
]
BASE = (sys.argv[1] if len(sys.argv) > 1 else "http://localhost:5173").rstrip("/")
OUT = ROOT / "docs/visual-review/batch-182"
...
print(json.dumps({"batch": 182, "routes": len(ROUTES), "modes": ["desktop", "mobile"]}))
