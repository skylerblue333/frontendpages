...
ROUTES=['/live','/live-chat','/live-streaming','/livestream-dashboard','/load-balancing','/loading-dialog','/log-viewer','/login','/m-l-insights','/m-l-models','/mailing-lists','/main-dashboard','/maintenance-mode','/map-view','/margin-trading','/market-sentiment','/marketing-r-o-i','/marketplace-analytics','/match-chat','/match-feed']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-194'
...
print(json.dumps({'batch':194,'routes':len(ROUTES),'modes':['desktop','mobile']}))
