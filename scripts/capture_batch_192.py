...
ROUTES=['/hope-a-i-advanced','/hope-a-i-page','/hope-a-i-upgrades','/hotel-search','/hub-spot-integration','/i-c-o-launchpad','/i-f-t-t-t','/image-gallery','/image-tools','/image-viewer','/in-app-notifications','/incident-management','/input-dialog','/instructor-dashboard','/integration-setup','/integrations','/inventory-management','/investment-goals','/investor-metrics','/investor-pitch']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-192'
...
print(json.dumps({'batch':192,'routes':len(ROUTES),'modes':['desktop','mobile']}))
