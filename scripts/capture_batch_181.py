...
ROUTES = [
    "/alert-configuration", "/alert-management", "/analytics", "/analytics-reports",
    "/anomaly-detection", "/anti-surveillance", "/approval-workflows", "/arbitrage-bot",
    "/archive-management", "/asset-tracking", "/assignment-tracker", "/audience-segmentation",
    "/audio-editing", "/batch-generation", "/beta", "/block-browser", "/blog-editor",
    "/blog-publisher", "/book-page", "/brand-guidelines",
]
BASE = (sys.argv[1] if len(sys.argv) > 1 else "http://localhost:5173").rstrip("/")
OUT = ROOT / "docs/visual-review/batch-181"
...
print(json.dumps({"batch": 181, "routes": len(ROUTES), "modes": ["desktop", "mobile"]}))
