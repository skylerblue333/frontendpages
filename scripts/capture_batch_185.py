...
ROUTES=['/cross-chain-interop','/cross-chain-swap','/crypto-exchange','/crypto-news','/crypto-research-hub','/d-a-o-treasury','/d-c-a-calculator','/d-m-inbox','/dashboard','/data-grid','/data-table','/data-visualization','/database-management','/date-input-form','/date-picker-dialog','/dating-discovery','/dating-home','/dating-messages','/dating-premium','/dating-profile']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-185'
...
print(json.dumps({'batch':185,'routes':len(ROUTES),'modes':['desktop','mobile']}))
