...
ROUTES=['/invoice-details','/invoice-management','/knowledge-base','/l-d-a-p-integration','/l-t-v-analysis','/language-exchange-admin','/language-settings','/lead-scoring','/leaderboards','/learning','/learning-path','/legal-documents','/lending-borrow','/lesson-editor','/lightbox','/like-reaction-system','/likes','/liquid-staking','/liquidity-pools','/list-view']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-193'
...
print(json.dumps({'batch':193,'routes':len(ROUTES),'modes':['desktop','mobile']}))
