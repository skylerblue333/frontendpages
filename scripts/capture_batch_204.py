...
ROUTES=['/shares','/sharing','/shipping-management','/shopping-cart','/sidebar-navigation','/sign-up','/sign-up-flow','/sign-up_old','/signin','/sky-school','/sky-school-a-i','/sky-store','/smart-contract-audit','/social-analytics','/social-events','/social-media','/stream-clip','/teaching-opportunities','/tier-comparison','/todo-list']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-204'
...
print(json.dumps({'batch':204,'routes':len(ROUTES),'modes':['desktop','mobile']}))
