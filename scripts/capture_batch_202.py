...
ROUTES=['/salesforce-integration','/satisfaction-survey','/saved-properties','/saved-searches','/savings-goals','/scheduled-jobs','/scheduled-reports','/school','/school-certificate','/school-course','/school-dashboard','/school-lesson','/school-quiz','/search','/search-analytics','/search-history','/search-results','/search-suggestions','/seasonal-events','/security']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-202'
...
print(json.dumps({'batch':202,'routes':len(ROUTES),'modes':['desktop','mobile']}))
