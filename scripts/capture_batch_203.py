...
ROUTES=['/security-audit','/security-compliance','/security-dashboard','/security-settings','/segmentation-analysis','/select-dropdown-form','/self-healing-infra','/seller-dashboard','/seller-profile','/send-crypto','/sentiment-pipeline','/server-health','/server-installer','/server-status','/settings','/settings-dialog','/setup-wizard','/shadow-identity','/shadow-relay','/share-dialog']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-203'
...
print(json.dumps({'batch':203,'routes':len(ROUTES),'modes':['desktop','mobile']}))
