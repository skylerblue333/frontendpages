...
ROUTES = [
    "/chain-explorer", "/change-log", "/channel-customization", "/chart-analysis",
    "/chart-dashboard", "/chat-bot", "/chat-history", "/checkbox-group-form",
    "/checkout", "/checkout-flow", "/churn-prediction", "/classroom-management",
    "/client-libraries", "/closing-checklist", "/code-completion", "/code-quality-dashboard",
    "/color-picker-dialog", "/comments-section", "/company-simulator", "/competitive-radar",
]
BASE = (sys.argv[1] if len(sys.argv) > 1 else "http://localhost:5173").rstrip("/")
OUT = ROOT / "docs/visual-review/batch-183"
...
print(json.dumps({"batch": 183, "routes": len(ROUTES), "modes": ["desktop", "mobile"]}))
