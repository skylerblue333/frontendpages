...
ROUTES=['/gaming-for-charity','/gantt-chart','/gas-fee-estimator','/gas-tracker','/general-settings','/getting-started-guide','/global-search','/grade-book','/group-chat','/group-chats','/group-directory','/group-events','/group-management','/growth','/guilds','/h-i-p-a-a','/hashtag-search','/health-articles','/health-dashboard','/help-center']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-191'
...
print(json.dumps({'batch':191,'routes':len(ROUTES),'modes':['desktop','mobile']}))
