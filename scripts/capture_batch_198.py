...
ROUTES=['/order-tracking','/order-types','/pagination','/password-input-form','/password-reset','/payment-confirmation','/payment-infra','/payment-methods','/payment-setup','/payout-management','/performance-tuning','/permission-management','/persona-builder','/phone-verification','/platform-map','/playlist-management','/portfolio-comparison','/portfolio-optimization','/portfolio-overview','/portfolio-rebalance']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-198'
...
print(json.dumps({'batch':198,'routes':len(ROUTES),'modes':['desktop','mobile']}))
