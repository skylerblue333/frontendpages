...
ROUTES=['/engagement-metrics','/engagement-stats','/engineer','/enterprise-billing','/environment-management','/error503','/error-dialog','/error-tracking','/escrow-shop','/event-analytics','/event-calendar','/event-creation','/event-planner','/event-registration','/exercise-library','/expense-management','/expense-tracker','/experiment-tracker','/export-data','/f-a-q-management']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-188'
...
print(json.dumps({'batch':188,'routes':len(ROUTES),'modes':['desktop','mobile']}))
