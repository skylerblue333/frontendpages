...
ROUTES=['/follow-requests','/follow-unfollow','/follower-list','/followers-network','/forum-categories','/fundraiser-tools','/g-d-p-r','/g-t-m-strategy','/gain-loss-tracking','/game-blackjack','/game-block-builder','/game-chat','/game-crash','/game-crypto-quiz','/game-fi-quest-board','/game-lobby','/game-room','/game-settings','/game-slots','/game-token-tap']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-190'
...
print(json.dumps({'batch':190,'routes':len(ROUTES),'modes':['desktop','mobile']}))
