...
ROUTES = ['/compliance-checking','/component-showcase','/confirmation-dialog','/connected-apps','/contact-us-form','/content-flagging','/content-vault','/context-menu','/contract-a-b-i','/contribution-interface','/conversation-archive','/cost-allocation','/cover-photo','/create-article','/create-audio','/create-drop','/creator-economy','/creator-network','/creator-onboarding','/creator-studio']
BASE = (sys.argv[1] if len(sys.argv) > 1 else 'http://localhost:5173').rstrip('/')
OUT = ROOT / 'docs/visual-review/batch-184'
for mode, width, height in (('desktop',1440,1000),('mobile',390,844)):
    ...
    with (OUT/f'screenshot-results-{mode}.jsonl').open('w') as log:
        for i, route in enumerate(ROUTES,1):
            ...
            target=folder/f'{i:03d}-{slug}.png'; started=time.time()
...
print(json.dumps({'batch':184,'routes':len(ROUTES),'modes':['desktop','mobile']}))
