...
ROUTES = [
    "/account-settings", "/revenue-tracking", "/dev-ops", "/governance-wizard",
    "/content-scheduler", "/review-moderation", "/ghost-mode", "/return-management",
    "/reward-system", "/reviews", "/returns-refunds", "/rewards-monitoring",
    "/reviews-ratings", "/distribution-channels", "/document-signing",
    "/developer-protocol", "/document-editor", "/documentation", "/document-sharing",
    "/discord-integration",
]
BASE = (sys.argv[1] if len(sys.argv) > 1 else "http://localhost:5173").rstrip("/")
OUT = ROOT / "docs/visual-review/batch-179"
    ...
    with log_path.open("w", encoding="utf-8") as log:
        for index, route in enumerate(ROUTES, 1):
            ...
            cmd = [
                ...
                "--virtual-time-budget=5000", f"--screenshot={target}", BASE + route,
            ]
            started = time.time()
            try:
...
print(json.dumps({"batch": 179, "routes": len(ROUTES), "modes": ["desktop", "mobile"]}))
