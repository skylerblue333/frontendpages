...
ROUTES=['/progress-bar','/property-detail','/publishing-queue','/q-r-code-generator','/quick-actions','/quick-stats','/quiz-builder','/radio-button-form','/rating-system','/receipt-download','/receive-crypto','/recent-activity','/recommendations','/recommendations-feed','/referrals','/refund-requests','/report-dialog','/report-user','/reports-dashboard','/retention-engine']
BASE=(sys.argv[1] if len(sys.argv)>1 else 'http://localhost:5173').rstrip('/')
OUT=ROOT/'docs/visual-review/batch-200'
...
print(json.dumps({'batch':200,'routes':len(ROUTES),'modes':['desktop','mobile']}))
