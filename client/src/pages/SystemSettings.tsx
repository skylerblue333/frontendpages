...
const tabs = [
  ...
  { id: "notifications", label: "Notification policy", icon: Bell },
  { id: "security", label: "Access control", icon: KeyRound },
  { id: "sessions", label: "Sessions & devices", icon: History },
  { id: "wallet", label: "Financial boundary", icon: WalletCards },
  { id: "danger", label: "Destructive controls", icon: ShieldAlert },
];
export default function SystemSettings() {
  ...
  const [feedback, setFeedback] = useState(
    "No system settings contract connected"
  );
  const reset = () => {
    setTab("profile");
    setDisplayName("");
    ...
    setSaved(false);
    setShowGates(false);
    setFeedback("No system settings contract connected");
  };
  const save = () => {
    setSaved(true);
    setFeedback("System draft saved locally");
  };
  return (
    <div
      data-ui-polish="batch-203"
      className="min-h-screen bg-[#070a16] text-white"
    >
      <ScreenHero
        icon={SlidersHorizontal}
        eyebrow="System settings · Governance controls"
        title="Review system controls without implying they are authorized or applied."
        description="Review local system-configuration concepts with save/reset and evidence gates. No organization scope, administrator role, environment, secret store, policy engine, audit trail, deployment target, identity provider, session authority, wallet custody, or destructive action is connected."
        badge="System settings preview"
      >
        <div className="flex flex-wrap gap-2">
          ...
          >
            ...
            {saved ? "Draft saved locally" : "Save system draft"}
          </Button>
          <Button
        ...
        <ScreenStatGrid
          items={[
            ...
            },
            {
              label: "Financial",
              value: "Not connected",
              hint: "No custody source",
              icon: WalletCards,
              tone: "slate",
            },
          ]}
        />
        <ScreenPreviewBanner title="System settings evidence boundary">
          <strong>
            This is a local system-configuration preview, not evidence that any
            setting, identity, secret, policy, session, wallet, notification,
            deployment, or deletion request is authorized, persisted, published,
            or executed.
          </strong>{" "}
          Form controls, toggles, saved state, and disabled system actions are
          browser concepts. No administrator identity, authorization grant,
          environment variable, secret, audit event, deployment, policy effect,
          wallet address, balance, seed phrase, message delivery, account
          deletion, or security outcome is asserted.
        </ScreenPreviewBanner>
