  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="mx-auto max-w-7xl space-y-6 px-4 py-6 sm:px-6 lg:px-8">
        ...
        <Card className="border-border/70 bg-card/80"><CardContent className="flex flex-col gap-4 p-5 sm:flex-row sm:items-start"><div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-amber-500/10 text-amber-300" aria-hidden="true"><ShieldAlert className="size-5" /></div><div><div className="mb-2 flex flex-wrap items-center gap-2"><Badge variant="outline" className="border-amber-500/30 text-amber-300">Unauthenticated preview</Badge>{!isAuthenticated && <span className="text-xs text-muted-foreground">Live exchange actions remain locked</span>}</div><h2 className="font-semibold">Trading safety boundary</h2><p className="mt-1 text-sm leading-6 text-muted-foreground">No live price, order, success rate, user count, response time, fill, balance, payment, or settlement claim is shown until the corresponding service is verified. Never submit an order without confirming network, asset, amount, fees, slippage, signing, and final transaction status.</p></div></CardContent></Card>
