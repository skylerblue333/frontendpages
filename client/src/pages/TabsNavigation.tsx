...
export function TabsNavigation() {
  ...
  const [status, setStatus] = useState(
    "Tabs navigation service unavailable locally. No route, focus, analytics, preference, or navigation mutation was started."
  );

  const announceUnavailable = (action: string) =>
    setStatus(
      `${action} is unavailable locally. No route, focus, analytics, storage, or navigation mutation was started.`
    );
  const updateNotificationPreview = (key: NotificationKey) => {
    setNotifications(previous => ({ ...previous, [key]: !previous[key] }));
    setStatus(
      "Navigation preview changed locally only. It was not persisted, routed, announced to analytics, or sent to a provider."
    );
  };
      ...
      >
        <header className="space-y-3">
          ...
          <p className="text-muted-foreground">
            Review typed navigation, keyboard, focus, and content boundaries
            without claiming that any setting is connected or persisted.
          </p>
        </header>
        <section
          ...
          <div className="flex items-start gap-3">
            ...
            <div>
              <h2 className="font-semibold text-amber-100">
                Tab navigation service unavailable
              </h2>
              <p className="mt-1 text-sm leading-6 text-amber-100/75">
                No route registry, deep-link contract, focus manager, analytics
                sink, persistence layer, or authorization source is connected.
                Values shown here are local preview state and are not
                authoritative.
              </p>
            </div>
          </div>
        ...
        <nav
          aria-label="Tabs navigation sections"
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <div className="flex gap-2">
                    <Input
                      id="account-email"
                      value="Route registry unavailable"
                      disabled
                  ...
                  >
                    Route ownership and deep-link verification are unavailable.
                    No route value is read from browser storage and no active
                ...
                <div className="rounded-xl border border-border/30 bg-background/30 p-4 text-sm text-muted-foreground">
                  <Mail
                    ...
                    aria-hidden="true"
                  />
                  Accessible tab semantics require a route registry, focus
                  management, deep-link validation, and tested content
                  ownership.
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent>
                <form
                  ...
                  onSubmit={event => {
                    ...
                    announceUnavailable("Navigation update");
                  }}
                >
                  <div>
                    <label
