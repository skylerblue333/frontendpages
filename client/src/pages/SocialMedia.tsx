    ...
    <main className="container max-w-3xl py-8 space-y-5">
      <header>
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Posts shown here come from the persisted feed record. Reactions,
          comments, follows, and ranking remain unavailable until their
      ...
      {isAuthenticated && user ? (
        <Card className="p-4 space-y-3">
          ...
          <div className="flex items-center justify-between gap-3">
            <button
              type="button"
              aria-label={`Set post visibility preview to ${postPrivacy === "public" ? "followers" : "public"}`}
              onClick={() =>
                setPostPrivacy(value =>
                  value === "public" ? "followers" : "public"
                )
              }
            ...
            >
              ...
              {postPrivacy} visibility is informational until audience controls
              are implemented
            </button>
            <Button
              onClick={publish}
        ...
        <Card className="p-4 flex items-center justify-between gap-4">
          <p className="text-sm text-muted-foreground">
            Sign in to publish a persisted post.
          </p>
          <Link href="/login">
          ...
          <Card key={post.id} className="p-5 space-y-3">
            ...
            <div className="flex items-center gap-4 border-t pt-3 text-xs text-muted-foreground">
              ...
              <span className="ml-auto inline-flex items-center gap-1">
                <MessageCircle className="h-3.5 w-3.5" /> Interactions
                unavailable
              </span>
              <button
                type="button"
